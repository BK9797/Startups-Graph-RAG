...
with st.sidebar:
    ...
    model_override = st.text_input("Groq model override (optional)", value="", placeholder="llama-3.3-70b-versatile")
...
st.caption(
    "Ask a natural-language question. It's translated to Cypher, run read-only against Neo4j, "
    "and answered by an LLM grounded strictly in the retrieved graph data."
)
...
for turn in st.session_state.history:
    ...
    with st.chat_message("assistant"):
        ...
        tab_cypher, tab_results, tab_graph = st.tabs(["🔎 Generated Cypher", "📊 Raw Results", "🕸️ Graph View"])
        with tab_cypher:
...
if question:
    ...
    with st.chat_message("assistant"), st.spinner("Generating Cypher, querying Neo4j, and synthesizing an answer…"):
        try:
            t0 = time.time()
            data = call_answer(
